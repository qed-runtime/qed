...
既定のChatタブは最近のuserとassistant messageを保持してassistant textをstream表示し、重要なTool、approval、capacity、compaction、terminal activityをEvent sequenceなしでmessage間へ表示します
連続するActivityは1つの選択可能な複数行blockへまとめます
内部requestとstate capture EventはActivityタブだけに残し、保持する全ActivityをEvent sequence付きの1つの複数行選択可能documentとして表示します
F3またはtab選択で表示を切り替えます
...
raw Tool argument、Tool出力、raw wait payload、raw Run errorはどちらのtabでもrendering用の表示状態へ保持しません
...
各tabは再描画とtab切替後も個別のapplication所有selectionを維持します
Chat selectionは1つのmessageまたはgrouped Activity block内に限定し、Activity selectionは保持する全rowをまたげます
clickまたは左dragし、空でないselectionはCtrl-C、focus中のblockまたはActivity document全体はCtrl-Shift-CでOSC 52へcopyできます
F8は現在のtab全体を画面と同じ安全なtextとしてcopyします
tab copyは1 MiBまでで、上限へ達した場合は末尾に`[copy truncated]`を追加します
selectionがCtrl-Cを所有しない場合は従来どおりRunをcancelします
現在のfeedへfocusを移した後はPageUpとPageDownで内容を移動でき、mouse wheelはpointerがある領域をscrollします
